...
三种输入：**单人文本 / A/B 多人对话 / 上传 txt**。正文里可以插**停顿**（0–5 秒可调）和**副语言**（笑声、咳嗽、换气、叹气等），从别处粘贴带 `[停顿 2s]`、`[laughter]` 这类标记的文本会自动还原成可编辑的芯片。**语速、音调、音量、风格、风格强度**都能调，输出支持 **MP3 / WAV / OGG**。长文按句切分再拼回一段音频，生成后可试听或导出。
